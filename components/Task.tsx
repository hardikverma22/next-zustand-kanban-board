...
const Task = ({ id, title, description, status }: { id: string; title: string; description?: string; status: Status }) => {
      ...
      targetComponent={
        ...
        >
          ...
          <button className="cursor-pointer" onClick={() => removeTask(id)}>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="h-5 w-5 text-gray-500 hover:text-rose-400">
    ...
    >
      <form id="update-task-form" className="grid gap-4 py-4" onSubmit={handleSubmit}>
        <div className="grid grid-cols-4 items-center gap-4">
          <Input id="title" name="title" value={taskTitle} onChange={(e) => setTaskTitle(e.target.value)} className="col-span-4" />
        </div>
        <div className="grid grid-cols-4 items-center gap-4">
          <Textarea id="description" name="description" value={taskDesc} onChange={(e) => setTaskDesc(e.target.value)} placeholder="Description..." className="col-span-4" />
        </div>
      </form>
    </CustomDialog>
  );
